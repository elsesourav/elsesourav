import { PrismaClient, UserRole, PublishStatus, TicketStatus, TicketPriority } from '@prisma/client';
...
async function main() {
  ...
  const tagCLI = await prisma.tag.upsert({ where: { slug: 'cli' }, update: {}, create: { name: 'CLI', slug: 'cli' } });
  const tagWeb = await prisma.tag.upsert({ where: { slug: 'web' }, update: {}, create: { name: 'Web', slug: 'web' } });
  const tagReact = await prisma.tag.upsert({ where: { slug: 'react' }, update: {}, create: { name: 'React 19', slug: 'react' } });
  const tagTypeScript = await prisma.tag.upsert({ where: { slug: 'typescript' }, update: {}, create: { name: 'TypeScript', slug: 'typescript' } });
  const tagOpenSource = await prisma.tag.upsert({ where: { slug: 'open-source' }, update: {}, create: { name: 'Open Source', slug: 'open-source' } });
  const tagCloud = await prisma.tag.upsert({ where: { slug: 'cloud' }, update: {}, create: { name: 'Cloud', slug: 'cloud' } });
  ...
  const appTerminal = await prisma.app.upsert({
    ...
    create: {
      ...
      description: 'A cutting-edge WebGL-powered terminal emulator supporting split panes, custom themes, SSH tunnels, and lightning-fast rendering.',
      iconUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/terminal-pro-icon.png',
      featuredImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/terminal-pro-banner.png',
      ...
      seoDescription: 'High performance browser-based terminal emulator with split multiplexing and custom themes.',
      ...
      links: {
        create: [
          { platform: 'web', label: 'Launch Terminal', url: 'https://terminal.elsesourav.com', action: 'open_app', isPrimary: true, displayOrder: 0 },
          { platform: 'github', label: 'GitHub Repository', url: 'https://github.com/elsesourav/terminal-pro', action: 'view_source', isPrimary: false, displayOrder: 1 },
        ],
      },
      versions: {
        create: [
          { version: '2.1.0', changelog: 'Upgraded to Next.js 15, WebGL 2.0 rendering engine, and dynamic tab splits.' },
          { version: '2.0.0', changelog: 'Complete architecture redesign with TypeScript and zero-latency WebSocket stream.' },
        ],
      },
      stats: {
        create: { views: 4250, launches: 3120, libraryAdds: 890, ratingAverage: 4.95, ratingCount: 88 },
      },
    },
  });
  ...
  const appPalette = await prisma.app.upsert({
    ...
    create: {
      ...
      description: 'Generate harmonious color systems, test WCAG 2.1 AA/AAA accessibility contrast in real-time, and export directly to Tailwind CSS and CSS variables.',
      iconUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/palette-studio-icon.png',
      featuredImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/palette-studio-banner.png',
      ...
      seoDescription: 'Generate accessible color palettes, check WCAG contrast, and export tokens for Tailwind and Figma.',
      ...
      links: {
        create: [
          { platform: 'web', label: 'Open Palette Studio', url: 'https://palette.elsesourav.com', action: 'open_app', isPrimary: true, displayOrder: 0 },
          { platform: 'github', label: 'Source Code', url: 'https://github.com/elsesourav/palette-studio', action: 'view_source', isPrimary: false, displayOrder: 1 },
        ],
      },
      versions: {
        create: [
          { version: '1.4.0', changelog: 'Added OKLCH color space support and automatic dark mode counterpart generation.' },
          { version: '1.0.0', changelog: 'Initial release with HSL/Hex color generator and WCAG AA contrast matrix.' },
        ],
      },
      stats: {
        create: { views: 3100, launches: 2450, libraryAdds: 610, ratingAverage: 4.88, ratingCount: 52 },
      },
    },
  });
  ...
  const appFocus = await prisma.app.upsert({
    ...
    create: {
      ...
      description: 'Stay in the zone with smart interval timing, ambient soundscapes, task sprint tracking, and GitHub commit streak integrations.',
      iconUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/focusflow-icon.png',
      featuredImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/focusflow-banner.png',
      ...
      seoDescription: 'Productivity timer designed for engineers with distraction blocking and task telemetry.',
      ...
      links: {
        create: [
          { platform: 'web', label: 'Launch Timer', url: 'https://focus.elsesourav.com', action: 'open_app', isPrimary: true, displayOrder: 0 },
        ],
      },
      versions: {
        create: [
          { version: '1.2.0', changelog: 'Offline PWA support and customizable ambient audio tracks.' },
        ],
      },
      stats: {
        create: { views: 1890, launches: 1420, libraryAdds: 410, ratingAverage: 4.75, ratingCount: 39 },
      },
    },
  });
  ...
  const appDevDock = await prisma.app.upsert({
    ...
    create: {
      ...
      description: 'Manage Docker containers, PostgreSQL databases, and local server instances from a single unified web dashboard.',
      iconUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/devdock-icon.png',
      featuredImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/devdock-banner.png',
      ...
      links: {
        create: [
          { platform: 'web', label: 'Open DevDock', url: 'https://devdock.elsesourav.com', action: 'open_app', isPrimary: true, displayOrder: 0 },
        ],
      },
      versions: {
        create: [
          { version: '3.0.0', changelog: 'Docker compose multi-project manager and live container log streaming.' },
        ],
      },
      stats: {
        create: { views: 2780, launches: 1950, libraryAdds: 530, ratingAverage: 4.92, ratingCount: 64 },
      },
    },
  });
  ...
  const appRegexLens = await prisma.app.upsert({
    ...
    create: {
      ...
      description: 'Break down complex regular expressions into understandable visual state machines with real-time match highlighting and unit tests.',
      iconUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/regexlens-icon.png',
      featuredImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/apps/regexlens-banner.png',
      ...
      links: {
        create: [
          { platform: 'web', label: 'Test Regex', url: 'https://regex.elsesourav.com', action: 'open_app', isPrimary: true, displayOrder: 0 },
        ],
      },
      versions: {
        create: [
          { version: '1.1.0', changelog: 'Added ECMAScript 2024 named capture group visualization.' },
        ],
      },
      stats: {
        create: { views: 1650, launches: 1100, libraryAdds: 320, ratingAverage: 4.82, ratingCount: 28 },
      },
    },
  });
  ...
  const blogCatEng = await prisma.blogCategory.upsert({
    ...
    update: {},
    create: { name: 'Engineering', slug: 'engineering', description: 'Technical deep-dives and systems architecture.' },
  });

  const blogCatArch = await prisma.blogCategory.upsert({
    where: { slug: 'architecture' },
    update: {},
    create: { name: 'Architecture', slug: 'architecture', description: 'Scalable system design and software patterns.' },
  });

  const blogCatRel = await prisma.blogCategory.upsert({
    where: { slug: 'releases' },
    update: {},
    create: { name: 'Product Releases', slug: 'releases', description: 'Platform updates and major milestone releases.' },
  });

  const blogTagNext = await prisma.blogTag.upsert({ where: { slug: 'nextjs-15' }, update: {}, create: { name: 'Next.js 15', slug: 'nextjs-15' } });
  const tagPostgres = await prisma.blogTag.upsert({ where: { slug: 'postgresql' }, update: {}, create: { name: 'PostgreSQL', slug: 'postgresql' } });
  const tagSysDesign = await prisma.blogTag.upsert({ where: { slug: 'system-design' }, update: {}, create: { name: 'System Design', slug: 'system-design' } });
  const tagSec = await prisma.blogTag.upsert({ where: { slug: 'security' }, update: {}, create: { name: 'Security', slug: 'security' } });
  ...
  await prisma.blogPost.upsert({
    ...
    create: {
      ...
      excerpt: 'A comprehensive architectural journey migrating from Firebase to a high-performance Next.js 15 monorepo backed by PostgreSQL and Prisma.',
...
The result is a lightning-fast ecosystem that boots in under 100ms with full WCAG AA compliance.`,
      coverImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/blog/v2-architecture.png',
      status: PublishStatus.PUBLISHED,
      readingTime: 6,
      viewsCount: 1420,
      ...
      tags: { create: [{ tagId: blogTagNext.id }, { tagId: tagPostgres.id }, { tagId: tagSysDesign.id }] },
    },
  });
  ...
  await prisma.blogPost.upsert({
    ...
    create: {
      ...
      excerpt: 'How we implemented server-side authorization boundaries, input sanitization, and cryptographic token verification.',
...
\`\`\``,
      coverImageUrl: 'https://res.cloudinary.com/diqw2sjl8/image/upload/v1/blog/security-zero-trust.png',
      status: PublishStatus.PUBLISHED,
      readingTime: 5,
      viewsCount: 980,
  ...
  await prisma.blogPost.upsert({
    ...
    create: {
      ...
      excerpt: 'Creating a cohesive dark-mode design system with 25 reusable primitives, glassmorphism, and keyboard navigation.',
...
- You can request full account data deletion at any time from **Settings -> Danger Zone**.`,
  ...
  const ticket1 = await prisma.supportTicket.upsert({
    ...
    create: {
      ...
      description: 'When running Terminal Pro on a 4K display with fractional scaling, character borders sometimes blur on split panes.',
      category: 'bug',
        ...
        create: [
          {
            ...
            message: 'Hi team, I noticed fractional scaling on Linux causes text blurring in the Terminal Pro split view.',
            createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000),
          },
          {
            senderUserId: staffUser.id,
            senderRole: UserRole.STAFF,
            message: 'Thank you for reporting this Sarah! We reproduced this on Wayland with 125% scaling. A canvas pixel ratio fix is currently in testing.',
            createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
          },
          {
            senderUserId: adminUser.id,
            senderRole: UserRole.ADMIN,
            message: 'Internal Note: PR #42 patches this with window.devicePixelRatio listener. Will deploy in v2.1.1.',
            isInternalNote: true,
      ...
      messages: {
        create: [
          ...
          },
          {
            senderUserId: staffUser.id,
            senderRole: UserRole.STAFF,
            message: 'Good news David! We added SVG export in Palette Studio v1.4.0. You can now copy or download raw SVG swatches directly.',
  ...
  await prisma.supportTicket.upsert({
    ...
    create: {
      ...
      description: 'Are all color palettes generated in Palette Studio guaranteed to meet WCAG AAA contrast?',
      category: 'general',
        ...
        create: [
          {
            ...
            senderRole: UserRole.USER,
            message: 'Hello! I wanted to confirm if the contrast matrix checks both light and dark backgrounds automatically.',
            createdAt: new Date(),
          },
        ],
      },
  ...
  await prisma.appFeedback.createMany({
    data: [
      { userId: userSarah.id, appId: appTerminal.id, rating: 5, message: 'Phenomenal web terminal. The WebGL rendering is butter smooth!', status: 'approved' },
      { userId: userDavid.id, appId: appDevDock.id, rating: 5, message: 'Saved me hours managing local microservices.', status: 'approved' },
      { userId: userElena.id, appId: appPalette.id, rating: 5, message: 'The contrast checker makes WCAG AA compliance effortless.', status: 'approved' },
      { userId: userSarah.id, appId: appFocus.id, rating: 4, message: 'Great timer! Would love integrations with Spotify.', status: 'approved' },
    ],
    skipDuplicates: true,
  });
  ...
  console.info('  • Applications: 5 Full Dev Tools (Terminal Pro, Palette Studio, FocusFlow, DevDock, RegexLens)');
