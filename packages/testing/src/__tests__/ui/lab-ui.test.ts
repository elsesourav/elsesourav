...
describe('Public Lab Experience Integration', () => {
  const mockLabApps: AppListItem[] = [
    {
      id: 'app-lab-1',
      slug: 'falling-sands',
  ...
  it('filters out non-simulation apps to isolate authentic Lab experiments', () => {
    const labApps = mockLabApps.filter(
      (app) => app.categorySlug === 'simulations' || app.primaryCategory.toLowerCase().includes('simulation')
    );

    expect(labApps).toHaveLength(3);
    expect(labApps.map((a) => a.slug)).toEqual([
      'falling-sands',
  ...
  it('supports topic filtering for WASM and C++ experiments', () => {
    const wasmApps = mockLabApps.filter(
      (app) =>
  ...
  it('supports procedural and algorithm filtering', () => {
    const algoApps = mockLabApps.filter(
      (app) =>
  ...
  it('verifies that moving a project from Lab to Work is driven by database classification without duplicate records', () => {
    const reclassifiedApp: AppListItem = {
      ...mockLabApps[0]!,
      primaryCategory: 'Creative Tools',
      categorySlug: 'creative-tools',
    };

    const isStillInLab = reclassifiedApp.categorySlug === 'simulations';
    expect(isStillInLab).toBe(false);
  });
});
