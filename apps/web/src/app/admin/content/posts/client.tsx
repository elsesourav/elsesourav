...
type PostStatus = AdminPost["status"];
...
const postStatusOptions: PostStatus[] = ["DRAFT", "PUBLISHED", "ARCHIVED"];
...
function toSortedPosts(items: AdminPost[]): AdminPost[] {
  return [...items].sort(
    (a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
  );
}

function toSortedTags(items: AdminPostTag[]): AdminPostTag[] {
...
function createPostFormFromItem(item: AdminPost): BlogPostFormState {
  return {
    slug: item.slug,
    title: item.title,
...
function validatePostForm(form: BlogPostFormState): string | null {
  ...
  if (!postStatusOptions.includes(form.status)) {
...
function mergePostPatch(
  previous: AdminPost,
  patch: Partial<AdminPost>,
): AdminPost {
  return {
    ...previous,
    ...patch,
        ...
        <article className="max-h-40 overflow-auto rounded-xl border border-black/10 bg-[#fbfcff] p-3">
          {tags.length === 0 ? (
            <p className="text-xs text-[#5a647d]">
              Create tags in the tag panel to classify posts.
            </p>
          ) : (
...
export function AdminContentPostsClient({
  initialPosts,
  initialTags,
}: {
  initialPosts: AdminPost[];
  initialTags: AdminPostTag[];
}) {
  ...
  async function onCreateTag(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/admin/content/posts/tags", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !isApiSuccess<AdminPostTag>(payload)) {
        setTagError(parseApiMessage(payload) ?? "Failed to create post tag.");
        return;
      }
  ...
  async function onDeleteTag(tag: AdminPostTag) {
    setDeletingTagId(tag.id);

    try {
      const response = await fetch(`/api/admin/content/posts/tags/${tag.id}`, {
        method: "DELETE",
      });
      ...
      if (!response.ok || !isApiSuccess<{ deleted: boolean }>(payload)) {
        dispatch(
          enqueueNotification({
            tone: "error",
            message: parseApiMessage(payload) ?? "Failed to delete post tag.",
          }),
        );
        return;
      }
  ...
  async function onCreatePost(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/admin/content/posts/posts", {
        method: "POST",
        headers: {
          "content-type": "application/json",
        },
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok || !isApiSuccess<AdminPost>(payload)) {
        setCreateError(
          parseApiMessage(payload) ?? "Failed to create post.",
        );
        return;
      }
  ...
  async function onSavePostEdits(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        `/api/admin/content/posts/posts/${editingPostId}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok || !isApiSuccess<AdminPost>(payload)) {
        setEditError(parseApiMessage(payload) ?? "Failed to update post.");
        return;
      }
  ...
  async function onConfirmPublishPost() {
    ...
    try {
      const response = await fetch(
        `/api/admin/content/posts/posts/${confirmPublishId}/publish`,
        {
          method: "POST",
        },
      );
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok || !isApiSuccess<Partial<AdminPost>>(payload)) {
        dispatch(
          enqueueNotification({
            tone: "error",
            message: parseApiMessage(payload) ?? "Failed to publish post.",
          }),
        );
        return;
      }
        ...
        toSortedPosts(
          previous.map((item) =>
            item.id === confirmPublishId
              ? mergePostPatch(item, payload.data)
              : item,
          ),
        ),
      );
  ...
  async function onConfirmArchivePost() {
    ...
    try {
      const response = await fetch(
        `/api/admin/content/posts/posts/${confirmArchiveId}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!response.ok || !isApiSuccess<Partial<AdminPost>>(payload)) {
        dispatch(
          enqueueNotification({
            tone: "error",
            message: parseApiMessage(payload) ?? "Failed to archive post.",
          }),
        );
        return;
      }
      ...
      <Card className="space-y-4">
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(["ALL", ...postStatusOptions] as const).map((status) => (
              <button
                key={status}
                type="button"
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
        <section className="space-y-4">
          {visiblePosts.length === 0 ? (
            <Card>
              ...
              <CardDescription className="mt-1">
                Adjust search or status filters, or create your first post.
              </CardDescription>
            </Card>
          ) : (
        ...
        <Card className="space-y-3">
          <div>
            ...
            <CardDescription className="mt-1">
              Create and remove post tags used for editorial organization.
            </CardDescription>
          </div>
