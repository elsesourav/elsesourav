            ...
            {isOAuth ? (
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Your email is linked to your {user.provider === 'google' ? 'Google' : 'GitHub'}{' '}
                  account and cannot be modified directly.
                </p>
              </div>
            ) : (
          ...
          <div className="p-4 rounded-2xl bg-muted/40 border border-border space-y-3">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="info" className="text-xs px-2.5 py-1">
                {isOAuth
                  ? `${user.provider === 'google' ? 'Google' : 'GitHub'} OAuth Identity`
                  : 'Supabase Email & Password'}
              </Badge>
            </div>
            <p className="text-[11px] text-muted-foreground">
              Account sessions are protected using secure HTTP-only cookies and cryptographic token
              validation.
            </p>
          </div>
